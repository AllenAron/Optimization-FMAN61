classdef Lab2 < matlab.apps.AppBase
    ...
    properties (Access = private)
        ...
        d2String = '$\\mathbf{x}_k = (%.4f\\,%.4f)^T$';
        d3String = '$\\mathbf{x}_k = (%.4f\\,%.4f\\,%.4f)^T$';
    ...
    methods (Access = private)
        ...
        function chooseFunctions(app)
            if app.SystemCheckBox.Value
                app.P = @(x) AG_penalty(app,x);
                app.B = @(x) AG_barrier(app,x);
                app.Mu.Visible = 'off';
                ...
                app.P = @(x) penalty(x,app.A,app.b);
                app.B = @(x) barrier(x,app.A,app.b);
                app.mu = 0;
                app.epsilon = 0;
        ...
        function updateF(app)
            if app.PenaltyCheckBox.Value
                app.F = @(x) app.c'*x + app.mu*app.P(x);
            else
                app.F = @(x) min(app.c'*x + app.epsilon*app.B(x),app.almostInf);
            end
        end
        ...
        function updateTheory(app)
            if app.PenaltyCheckBox.Value
                app.TheoryLabel.Text = "${\min \atop \scriptstyle \mathbf{x} \in \mathbb{R}^d} f(\mathbf{x}) + \mu_k P(\mathbf{x})$";
            else
                app.TheoryLabel.Text = "${\min \atop \scriptstyle \mathbf{x} \in \mathrm{int}(S)} f(\mathbf{x}) + \varepsilon_k B(\mathbf{x})$";
            end
        end
    end
